...
prepare()
{
	...
	cp -r icon-themes/oxygen/* icon-themes/export # prevent build failure build process will expect this folder with contents theme we are building with
}
...
configure()
{
	...
	ln -sv ./src/libreoffice-translations-5.3.2.2/translations ./
}
...
install(){
	...
	# TODO: try to make it work, and use https://github.com/LibreOffice/core/tree/master/sysui to create desktop files and icons in the destination directory
	...
	ln -sv /opt/$package_name/program/sbase /bin
        ...
        # the following rewrites the *.desktop files so they can run libreoffice via hutchr, because we are not running make install
