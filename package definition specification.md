...
`rel_url`: the RELATIVE suffix-part of URL to package. Default value is `$package_name/$package_fullname.tar.$package_suffix`. Need to be used, when relative tar archive location is not matched with default value. SHOULD NOT contain "http://mirrors.tassig.com" prefix in its value.
